...
function selection2(id, conn, td){
    const sql1 = "SELECT u.u_id FROM User u, Characters c WHERE u.c_id = c.c_id AND u.c_id IN (SELECT id FROM Other WHERE c_id= (SELECT c_id FROM User WHERE u_id= ?) AND similar=1 order by rand()) AND u.u_id NOT IN (SELECT receiver FROM Destiny WHERE sender = ?) order by rand() limit 1";
...
function selection3(id, conn, td){
    const sql1 = "SELECT u.u_id FROM User u, Characters c WHERE u.c_id = c.c_id AND u.c_id IN (SELECT id FROM Other WHERE c_id= (SELECT c_id FROM User WHERE u_id= ?) AND similar=0 order by rand()) AND u.u_id NOT IN (SELECT receiver FROM Destiny WHERE sender = ?) order by rand() limit 1";
    conn.query(sql1, [id, id], (err, result) =>{
        const u_id = result[0].u_id;
